...
interface TeamContextType {
  ...
  deleteFacilityField: (facilityId: string, fieldId: string) => Promise<void>;
...
export function TeamProvider({ children }: { children: ReactNode }) {
  ...
  const respondToAssignment = useCallback(async (contextId: string, entryId: string, status: 'accepted' | 'declined') => { 
    ...
    if (leagueData && status === 'accepted') {
      ...
      // Note: arrayRemove and arrayUnion cannot be applied to the same field in a single update.
      // We'll need a second update or just rely on the union if the placeholder is handled differently.
      batch.update(leagueRef, { memberTeamIds: arrayRemove(placeholderKey) });
  ...
  const deleteTeamDocument = useCallback(async (docId: string) => { if (activeTeam?.id && db) await deleteDoc(doc(db, 'teams', activeTeam.id, 'documents', docId)); }, [activeTeam, db]);
  ...
  const deleteFundraisingOpportunity = useCallback(async (fundId: string) => { if (activeTeam?.id && db) await deleteDoc(doc(db, 'teams', activeTeam.id, 'fundraising', fundId)); }, [activeTeam, db]);
  ...
  const updateGame = useCallback(async (gameId: string, data: any) => { if (activeTeam?.id && db) await updateDoc(doc(db, 'teams', activeTeam.id, 'games', gameId), clean(data)); }, [activeTeam, db]);
  const canAddProTeam = proQuotaStatus.remaining > 0;
  ...
  const publicSignUpForVolunteer = useCallback(async (teamId: string, oppId: string, data: any) => { if (db) await updateDoc(doc(db, 'teams', teamId, 'volunteers', oppId), { [`signups.${data.userId || Date.now()}`]: { ...data, isConfirmed: false, status: 'pending', createdAt: new Date().toISOString() } }); }, [db]);
  ...
  const resolveQuota = useCallback(async (selectedTeamIds: string[]) => { if (!db || !userProfile?.id) return; const batch = writeBatch(db); const ownedProTeams = teamsRaw.filter(t => t.isPro && t.ownerId === userProfile.id); ownedProTeams.forEach(t => { if (!selectedTeamIds.includes(t.id)) { batch.update(doc(db, 'teams', t.id), { isPro: false, planId: 'starter_squad' }); } }); await batch.commit(); }, [db, userProfile, teamsRaw]);
  ...
  const contextValue = useMemo(() => ({
    ...
    deleteTeamDocument, deleteFundraisingOpportunity, addGame, updateGame, canAddProTeam, deleteVolunteerOpportunity, publicSignUpForVolunteer
  }), [
